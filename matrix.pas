...
begin
    ...
    Dialogue.Say(Sanya, 'Что такое Симулятор Шобунена...');
    Dialogue.Say(MatrixRita,
        'Ответ там, Саня. И он ищет тебя и найдёт, если ты захочешь.');
    NextSlide;
    Dialogue.Say(MatrixKostyl,
    ...
    begin
        NextSlide;
        Dialogue.Say(MatrixTrip, 'Ты веришь в судьбу, Саня?');
        Dialogue.Say(Sanya, 'Нет.');
        Dialogue.Say(MatrixTrip, 'Почему?');
        Dialogue.Say(Sanya, 'Мобильная гача уничтожила эту франшизу...');
    end;
    NextSlide;
    Dialogue.Say(MatrixRita, 'Ты учил меня на Варшавку не соваться.');
    Dialogue.Say(MatrixTrip, 'Я надеюсь... что ошибался.');
    NextSlide;
    Dialogue.Say(MatrixKostyl,
    ...
    Dialogue.Say(MatrixTrip, 'Всегда есть выбор.');
    Dialogue.Say(MatrixRoma,
    ...
    Dialogue.Say(Sanya, 'Меня зовут... Саня!');
    NextSlide;
    TextToSpeech.Init;
    Dialogue.Close;
    TextToSpeech.Architect(NewLine + 'Здравствуй, Саня');
    Dialogue.FastSay(Sanya, 'Кто ты такой?');
